...
/// Shared HTTP client: JSON requests, Bearer tokens, and SimpleJWT refresh.
class ApiClient {
  ApiClient({
  ...
  Future<http.Response> sendMultipart(
    ...
    try {
      ...
      throw ApiException('Could not reach the Django server. ${error.message}');
    }
  }

  /// Public wrapper used when a multipart upload gets HTTP 401.
  Future<bool> tryRefresh() => _refreshAccessToken();
  ...
  void ensureSuccess(http.Response response, {Set<int> extraOk = const {}}) {
    final ok = response.statusCode >= 200 && response.statusCode < 300;
    if (ok || extraOk.contains(response.statusCode)) {
      return;
    }
